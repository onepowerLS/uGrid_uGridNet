...
hamlet_name = "LEB_6_BeseLaTeng"
concession = 'LEB_6_BeseLaTeng'
...
def DistanceBWindexes(indexesA, indexesB, d_EW_between, d_NS_between):
    # if isinstance(indexesA[0], list):
    #     indexesA = indexesA[0]
    #
    # if isinstance(indexesB[0], list):
    #     indexesB = indexesB[0]
    ...
    if (isinstance(indexesA, list) and len(indexesA) == 2):
        # print("A: ")
        # print(indexesA[0])
        # print("B: ")
        # print(indexesB[0])
...
def PolePlacement(reformatScaler, num_clusters, exclusionBuffer, range_limit, indexes_conn, indexes_excl, height, width,
    ...
    for i in range(len(index_pole_comp)):
        if index_pole_comp[i] in index_excl_comp:
            ...
            index_x_new, index_y_new = FindNonExclusionSpot(initial_pole_placement[i, 0], initial_pole_placement[i, 1],
                                                            index_excl_comp, range_limit, height, width)
...
def CollectVillageData():
    ...
    # print(hamlet_name + '_connections.xlsx')
    Connect_nodes = pd.read_excel(hamlet_name + '_connections.xlsx')
    Exclusion_nodes = pd.read_excel(hamlet_name + '_exclusions.xlsx', sheet_name='exclusions')

    # Identify gps coordinate min and max to determine coordinates of edges of jpg image
    Longitude_exc = Exclusion_nodes['Latitude']
    Latitude_exc = Exclusion_nodes['Longitude']
    ...
    pages = convert_from_path(hamlet_name + '_exclusions.pdf', 500)
    exclusions_jpg_ = hamlet_name + '_exclusions.jpg'
    for page in pages:
        page.save(exclusions_jpg_, 'JPEG')

    # Convert JPG to array
    ExclusionMap = Image.open(exclusions_jpg_)
    ...
    try:
        ...
        for i in range(len(Connect_nodes)):  # iteration through connections
            d_Econnection[i] = GPStoDistance(Lat_exc_min, Lat_exc_min, Long_exc_min,
                                             m.radians(Connect_nodes['Latitude'][i]))  # m
            # print(d_Econnection[i])
            # distance of connection to the east (left) (x index)
            d_Nconnection[i] = GPStoDistance(Lat_exc_min, m.radians(Connect_nodes['Longitude'][i]), Long_exc_min,
                                             Long_exc_min)  # m
...
def Plot_AllPoles_AllWiring(POI, OnOff_MV, indexes_poles_MV, OnOff_groups, indexes_poles_groups, indexes_conn,
                            ConnPoles, indexes_poles_LV_all):
    ...
    for i in range(num_poles_MV):
        for j in range(i):
            if OnOff_MV[i, j] == 1:
                match = np.array([[indexes_poles_MV[i, 0], indexes_poles_MV[i, 1]],
                                  [indexes_poles_MV[j, 0], indexes_poles_MV[j, 1]]])
                if goodToGo == 0:
                    goodToGo = 1
            ...
            for i in range(num_poles_group):
                for j in range(i):
                    if OnOff_group[i, j] == 1:
                        match = np.array(
                            [[indexes_group[i][0], indexes_group[i][1]], [indexes_group[j][0], indexes_group[j][1]]])
                        if goodToGo == 0:
                            goodToGo = 1
...
def Check_connections(i, visited, OnOff):
    ...
    for j in range(len(OnOff[i, :])):
        if OnOff[i, j] == 1 and visited[
            j] == 0:  # if j in a pole (999 is default for not a pole, and that pole is not visited)
            visited[j] = 1
...
# ==============================================================================
# Wiring Algorithm
def WiringAlg(ConnPoles, prob, Cost_kWh, restoration_time, indexes_poles, wiring_cost, d_EW_between, d_NS_between,
              standalone):
    ...
    DistancesBWPoles_sorted = np.sort(DistancesBWPoles,
                                      axis=1)  # First value in row will be 0, need to skip first column
    ...
    while goodToGo == 0:
        ...
        for i in range(num_poles):
            for j in range(1,
                           num_conn_per_pole):
                # start at 1 to avoid first column of zeros. Nothing is stopping this from going over num_poles
                # if i >= len(DistancesBWPoles_sorted):
                ind = np.where(DistancesBWPoles == DistancesBWPoles_sorted[
                    i, j])  # This is going to give two values, need adjust so only one half of matrix is considered
                ...
                OnOff[y, x] = 1
        # Check for islands
        components, visited = ConnectedComponents(
            OnOff)  # need to make sure OnOff is being filled in way that there aren't rows of zeros
        if components == 1:
            # Calculate Load loss risk
            Edges = LineLosses(OnOff, ConnPoles, num_poles, Long_exc_min, Lat_exc_min, d_EW_between, d_NS_between,
                               indexes_poles)
    ...
    for k in range(len(DistancesBWPoles_in_use) - 1, -1, -1):
        ...
        if components == 1:  # no islands, Only move forward with best solution if no islands
            # Calculate Load loss risk
            Edges = LineLosses(OnOff_temp, ConnPoles, num_poles, Long_exc_min, Lat_exc_min, d_EW_between, d_NS_between,
                               indexes_poles)
            total_load_loss_risk = sum(Edges[:, 2])
            Reliability_Risk_Cost = total_load_loss_risk * prob * Cost_kWh * restoration_time
            # Solve for total distance and calc cost
...
def ClassifyNetworkPoles(concession, gen_LAT, gen_LON, gen_site_indexes, indexes_MV_Poles_wPOI, group_indexes_LV,
    ...
    all_MV_Poles = MVIntermediatePoles(mv_poles, OnOff_MV, d_BW_Poles_MV, d_BW_Adj_Poles, index_excl_comp, range_limit,
                                       max_y, max_x)
        ...
        for a in A:
            ...
            if len(del_idx_x) == 2 and del_idx_x[0] == del_idx_x[1]:
                ...
                for b_ in branches_:
                    ...
                    LV_Poles_names += [base + chr(sub_network_counter) + chr(branch_counter) + str(m + 1) for m in
                                       range(len(b_))]
                branch_counter += 1
    ...
    el, utm_x, utm_y, gps = PoleElevation(gen_LON, gen_LAT, gen_site_indexes, target_indexes, d_EW_between,
                                          d_NS_between)
    DF['elevation'] = el
    DF['UTM_X'] = utm_x
    DF['UTM_Y'] = utm_y
...
if __name__ == "__main__":
    ...
    reformatScaler = int(
        Net_Parameters['reformatScaler'][0])  # parameter to decrease the resolution of image (speeds up processing)
    exclusionBuffer = int(Net_Parameters['exclusionBuffer'][
                              0])  # meters that poles need to be form exclusions (other poles, exclusions, and connections)
    MaxDistancePoleConn = int(
        Net_Parameters['MaxDistancePoleConn'][0])  # (m) the maximum distance allowed for a pole to be from a connection
    ...
    # Community Load Data
    LoadKW_MAK = pd.read_excel('02082021_1621_LEB_8760_C073.xlsx', usecols='B', sheet_name='8760', header=0)
    # print(LoadKW_MAK)
    global kW_max
    kW_max = max(LoadKW_MAK['kW'])
    ...
    for i in range(minPoles, maxPoles):
        for nrep_LV in range(repeats_LV_poles):
            ...
            ConnPoles_LV, indexes_poles_LV = PolePlacement(reformatScaler, i, exclusionBuffer, range_limit,
                                                           indexes_conn, indexes_excl, height, width, d_EW_between,
                                                           d_NS_between)
            ...
            while nrep_MV < repeats_MV_poles and NoDecrease == 0:
                print("Number of Poles: " + str(i) + ",and LV repeat is " + str(nrep_LV + 1) + " out of " + str(
                    repeats_LV_poles) + ", MV repeat is " + str(nrep_MV + 1) + " out of " + str(repeats_MV_poles) + ".")
                t_temp = time.time()
                ...
                while load_behind > LV_kW_safety:
                    ...
                    for repeat_num in range(repeats_MV_clusters):
                        ConnPoles_MV, indexes_poles_MV = PolePlacement(reformatScaler, MV_pole_num, exclusionBuffer,
                                                                       range_limit, indexes_poles_LV, indexes_excl,
                                                                       height, width, d_EW_between, d_NS_between)
                        # Calculate load behind by clusters
                        load_behind, ConnPoles_LVMV = loadBehindPoles(ConnPoles_MV, ConnPoles_LV, num_conns,
                                                                      MV_pole_num, load_per_conn)
                ...
                closest_pole, indexes_gen, connection_point = POI_Pole(lat_Generation, long_Generation, Long_exc_min,
                                                                       Lat_exc_min, d_EW_between, d_NS_between,
                                                                       indexes_poles_MV, 50)

                # add POI_MV to wiring for MV
                indexes_Poles_MV_wPOI = np.concatenate((indexes_poles_MV, [indexes_gen]), axis=0)
                total_distance_MV, OnOff_MV, DistancesBWPoles_MV, num_conn_per_pole_MV, reliability_cost_MV, wire_cost_MV, total_time_MV = WiringAlg(
                    ConnPoles_LVMV, probability_MV, Cost_kWh, restoration_time_MV, indexes_Poles_MV_wPOI,
                    wiring_cost_MV, d_EW_between, d_NS_between, "False")
                ...
                for MV_pole in range(MV_pole_num):
                    ...
                    temp_group_indexes = np.vstack(
                        (temp_group_indexes, indexes_poles_MV[MV_pole, :]))  # add MV pole location to end
                ...
                for MV_pole_group in range(MV_pole_num):
                    total_distance_temp, OnOff_temp, DistancesBWPoles_temp, num_conn_per_pole_temp, reliability_cost_temp, wire_cost_temp, total_time_temp = WiringAlg(
                        ConnPoles_LV[:, 0], probability_LV, Cost_kWh, restoration_time_LV,
                        np.array(group_indexes[MV_pole_group]), wiring_cost_LV, d_EW_between, d_NS_between, "False")
                ...
                poleclasses, networklines, droplines, connections = ClassifyNetworkPoles(concession, lat_Generation,
                                                                                         long_Generation,
                                                                                         indexes_gen,
                                                                                         indexes_Poles_MV_wPOI,
                                                                                         group_indexes,
                                                                                         OnOff_MV, DistancesBWPoles_MV,
                                                                                         indexes_excl,
                                                                                         70, range_limit, width, height,
                                                                                         d_EW_between,
