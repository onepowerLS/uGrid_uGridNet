...
#==============================================================================
# Wiring Algorithm
def WiringAlg(townname,ConnPoles,prob,Cost_kWh,restoration_time,indexes_poles,wiring_cost,d_EW_between,d_NS_between,standalone):
    t0 = time.time()

    #Load solution pole indexes
    if standalone == "True":
        filename = "%s_indexes_poles_reformatted_%s_soln.csv" %(townname,str(reformatScaler))
    ...
    goodToGo =  0
    num_conn_per_pole = 2 #starting value
    while goodToGo == 0:
...
if __name__ == "__main__":
    ...
    repeats_MV_clusters = 3
    repeats_LV_clusters = 3
    ...
    for i in range(minPoles,maxPoles):
        for nrep_LV in range(repeats_LV_poles):
            ...
            while nrep_MV < repeats_MV_poles and NoDecrease == 0:
                ...
                total_distance_MV, OnOff_MV, DistancesBWPoles_MV, num_conn_per_pole_MV, reliability_cost_MV, wire_cost_MV, total_time_MV = WiringAlg(townname,ConnPoles_LVMV,probability_MV,Cost_kWh,restoration_time_MV,indexes_Poles_MV_wPOI,wiring_cost_MV,d_EW_between,d_NS_between,"False")
                ...
                for MV_pole_group in range(MV_pole_num):
                    total_distance_temp, OnOff_temp, DistancesBWPoles_temp, num_conn_per_pole_temp, reliability_cost_temp, wire_cost_temp, total_time_temp = WiringAlg(townname,ConnPoles_LV[:,0],probability_LV,Cost_kWh,restoration_time_LV,np.array(group_indexes[MV_pole_group]),wiring_cost_LV,d_EW_between,d_NS_between,"False")
    ...
    OriginalExclusionsPlot(jpg_name) 

    PlotPolesConnsExclswithSolution(townname,reformatScaler,exclusionBuffer,indexes_gen, Best_OnOff_MV, Best_indexes_poles_MV, Best_OnOff_groups, Best_group_indexes, indexes_conn, Best_ConnPoles_LV, Best_indexes_poles_LV)    
