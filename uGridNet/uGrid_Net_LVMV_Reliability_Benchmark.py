...
def ExclusionMapper1(townname,ExclusionMap_array,reformatScaler,exclusionBuffer,d_EW_between,d_NS_between,width_new,height_new):
    ...
    #with images taken right from google earth pro, white is exclusion, anything else is up for grabs
    #This is reverse from the MAK
    ...
    #Save new indexes
    index_csv_name = "%s_indexes_reformatted_%s_bufferzone_%s.csv" %(townname,str(reformatScaler),str(exclusionBuffer))
...
def CollectVillageData(townname,Connect_nodes_name,jpg_name,exc_limits_name):
    #Load Files
    ...
    Connect_nodes = pd.read_excel(Connect_nodes_name, sheet_name = 'connections')
    #Exclusion_nodes = pd.read_excel(exclusion_nodes_name, sheet_name = 'exclusions')
            
    #Use limits worksheet to get lat min/max etc.
    Exc_limits = pd.read_excel(exc_limits_name, sheet_name = 'Sheet1')
    Lat_exc_min = m.radians(Exc_limits['lat min']) #top of image (north)
    Lat_exc_max = m.radians(Exc_limits['lat max']) #bottom of image (south)
    Long_exc_min = m.radians(Exc_limits['long min']) #left of image (east)
    Long_exc_max = m.radians(Exc_limits['long max']) #right of image (west)
    ...
    ExclusionMap = Image.open(jpg_name)
    ...
    try:
        ...
        #quit()
        indexes_excl = ExclusionMapper1(townname,ExclusionMap_array,reformatScaler,exclusionBuffer,d_EW_between,d_NS_between,width,height)

    #Match the connection locations to locations in the array
    #Find distance between east limit of image and connection
    try:
        #print("in try loop")
        index_csv_name = "%s_indexes_conn_reformatted_%s.csv" %str(townname,reformatScaler)
        ...
        for i in range(len(Connect_nodes)): #iteration through connections
            ...
            index_csv_name = "%s_indexes_conn_reformatted_%s.csv" %(townname,str(reformatScaler))
...
def Plot_AllPoles_AllWiring(probLV,probMV,POI,OnOff_MV, indexes_poles_MV, OnOff_groups, indexes_poles_groups, indexes_conn, ConnPoles, indexes_poles_LV_all):
    ...
    ax.set_aspect('equal')
    plotname = "AllPolesAllWiring_probLV_%s_probMV_%s.png" %(str(prob_LV),str(prob_MV))
...
def WiringAlg(ConnPoles,prob,Cost_kWh,restoration_time,indexes_poles,wiring_cost,d_EW_between,d_NS_between,standalone):
    ...
    goodToGo =  0
    num_conn_per_pole = 0 #starting value
    while goodToGo == 0:
...
def OriginalExclusionsPlot(jpg_name):
    #Import kml pdf file (of exclusions) and convert to jpg
    #pages = convert_from_path('MAK_exclusions.pdf',500)
    #for page in pages:
    #    page.save('MAK_exclusions.jpg','JPEG')
    ...
    fig = plt.imshow(ExclusionMap_array)
    fig.axes.get_xaxis().set_visible(False)
    fig.axes.get_yaxis().set_visible(False)
    
    plotname = "ExclusionsOriginal.png"
...
#==============================================================================
#Plot poles, connections, and exclusions
def PlotPolesConnsExclswithSolution(townname,reformatScaler,buffer,POI,OnOff_MV, indexes_poles_MV, OnOff_groups, indexes_poles_groups, indexes_conn, ConnPoles, indexes_poles_LV_all):
    fig, ax = plt.subplots()
    index_csv_name = "%s_indexes_conn_reformatted_%s.csv" %(townname,str(reformatScaler))
    indexes_conn = np.loadtxt(index_csv_name, delimiter=",")
    plt.scatter(indexes_conn[:,0],indexes_conn[:,1],s=2,c ='b',marker='+')
    #ax.set_aspect('equal') 
    #plt.show()
    ...
    #ax.set_aspect('equal')  
    #plotname = "RRTOriginalConnectionsPoles.png"
    #plt.savefig(plotname)
    ...
    for group in range(len(indexes_poles_groups)):
        ...
        if len(indexes_group) != 1: #if only one index, skip (it will be under MV pole anyway)
            ...
            for i in range(0,num_match_group-1,2):
                j = i + 1
                plt.plot(wiringMatrix_group[:,i],wiringMatrix_group[:,j],color = 'blue',linewidth = 1.5)# cmap(group),linewidth=1.5)
            for k in range(num_poles_group):
                plt.scatter(indexes_group[k][0],indexes_group[k][1],s=2,c='g')
    ...
    #fig, ax = plt.subplots()
    index_csv_name = "%s_indexes_reformatted_%s_bufferzone_%s.csv" %(townname,str(reformatScaler),str(buffer))
    indexes_excl = np.loadtxt(index_csv_name, delimiter=",")
    fig = plt.scatter(indexes_excl[:,0],indexes_excl[:,1],s=2,c ='white',marker= 's')
    fig.axes.get_xaxis().set_visible(False)
    fig.axes.get_yaxis().set_visible(False)
    plt.axis('off')
    ax.set_axis_off()
    ax.set_aspect('equal')  
    plotname = "FullSolution_noExc.png"
    plt.savefig(plotname)
    plt.show()
...
if __name__ == "__main__":
    ...
    townname = 'westford'
    connections_name = townname+'_Connections.xlsx'
    jpg_name = townname+'_image_cropped.jpg'
    exc_limits_name = townname+'_exc_limits.xlsx'
    ...
    Cost_kWh =  Net_Parameters['Cost_kWh'][0]*1.5
    ...
    Connect_nodes = pd.read_excel(connections_name, sheet_name = 'connections')
    num_conns = len(Connect_nodes)
    
    #Community Load Data
    #LoadKW_MAK = pd.read_excel('LoadKW_MAK.xlsx',index_col=None, header=None)
    #global kW_max
    
    kW_max = 0.1748*num_conns #Lesotho avg. house load (max) is 0.1748 kW
    ...
    restoration_time_LV = int(Net_Parameters['restoration_time_LV'][0])*10
    
    #Collect Village Data (connections and exclusion zones)
    indexes_conn, indexes_excl, height, width, d_EW_between, d_NS_between,Long_exc_max, Long_exc_min,Lat_exc_max, Lat_exc_min = CollectVillageData(townname,connections_name,jpg_name,exc_limits_name)
    ...
    probabilities = [0,0.05,0.1,0.25,0.5,0.75,1]
    probabilities_MV = [0]
    ...
    for i in range(minPoles,maxPoles):
        for nrep_LV in range(repeats_LV_poles):
            ...
            nrep_MV = 0
            for nrep_MV in range(repeats_MV_poles):#< repeats_MV_poles:# and NoDecrease == 0:
                ...
                #Plot different probabilities using the same poles
                for prob_LV in probabilities:
                    for prob_MV in probabilities_MV:
                        ...
                        total_distance_MV, OnOff_MV, DistancesBWPoles_MV, num_conn_per_pole_MV, reliability_cost_MV, wire_cost_MV, total_time_MV = WiringAlg(ConnPoles_LVMV,prob_MV,Cost_kWh,restoration_time_MV,indexes_Poles_MV_wPOI,wiring_cost_MV,d_EW_between,d_NS_between,"False")
                        ...
                        for MV_pole_group in range(MV_pole_num):
                            total_distance_temp, OnOff_temp, DistancesBWPoles_temp, num_conn_per_pole_temp, reliability_cost_temp, wire_cost_temp, total_time_temp = WiringAlg(ConnPoles_LV[:,0],prob_LV,Cost_kWh,restoration_time_LV,np.array(group_indexes[MV_pole_group]),wiring_cost_LV,d_EW_between,d_NS_between,"False")
                        ...
                        #Plot
                        Plot_AllPoles_AllWiring(prob_LV,prob_MV, indexes_gen, OnOff_MV, indexes_Poles_MV_wPOI, OnOff_groups, group_indexes, indexes_conn, ConnPoles_LV, indexes_poles_LV)
                        
                        #Save Data
                        Equipment_cost = Total_Cost - total_reliability_cost_LVMV
                        Save_Data = [total_reliability_cost_LVMV,Equipment_cost,Total_Cost,total_distance_MV,total_distance_LV]
                        filename = "Probability_Effects_probLV_%s_probMV_%s.png" %(str(prob_LV),str(prob_MV))
