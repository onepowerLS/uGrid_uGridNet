...
## MCASHFLOW FUNCTION =====================================================================================================
def mcashflow(tariff_hillclimb_multiplier, lifetime, f_pv, a_pv, f, a, Batt_life_yrs, equity_debt_ratio, term, loadkWh,
              interest_rate, loanfactor, PVkW, BattKWh, LEC, C1_pv, C1_LPG, Cost_bank, Cost_Propane_yr):
    ...
    LoanPrincipal[0] = ((C1_pv + C1_LPG) * loanfactor) * (
            1 - equity_debt_ratio)  # "The amount of project finance needed to cover capex and initial opex"
    ...
    if interest_rate > 0:
        finance = LoanPrincipal[0] / ((1 - (1 / (
                1 + interest_rate) ** term)) / interest_rate)  # "Calculates the amount in each year for loan repayment"
    ...
    for j in range(1, lifetime):
        ...
        M[j] = (f_pv * a_pv * C1_pv / lifetime) + (f_pv * (1 - a_pv) * C1_pv / lifetime ** 2) * (2 * j - 1) + (
                f * a * C1_LPG / lifetime) + (f * (1 - a) * C1_LPG / lifetime ** 2) * (2 * j - 1)
...
def Econ_total(propane, PVkW, BattKWh, Batt_kWh_tot, peakload, loadkWh):
    ...
    Batt_life_yrs = np.floor((BattKWh * Econ_Parameters['Batt_lifecycle'][0]) / (
            Batt_kWh_tot + 0.01))  # "Years of battery life before replacement is necessary, rounded down to an integer"
    ...
    Cost_Dist = Econ_Parameters['Cost_Dist_wire'][0] * Econ_Parameters['Dist_km'][0] + \
                Econ_Parameters['Cost_Step_up_Trans'][0] * Econ_Parameters['Step_up_Trans_num'][0] + \
                Econ_Parameters['Cost_Pole_Trans'][0] * Econ_Parameters['Pole_Trans_num'][0] + \
                Econ_Parameters['Cost_Pole'][0] * Pole_num

    Cost_BOS = Cost_bank + Cost_inv + Econ_Parameters['Cost_control'][
        0] + Cost_Dist + Cost_Smartmeter + Cost_MPesa + Cost_charge_controllers  # "Balance of System"

    Cost_EPC = Cost_EPC_tracker + Econ_Parameters['Cost_EPC_LPG_tank'][0] + Econ_Parameters['Cost_EPC_Power_house'][0] + \
               Econ_Parameters['Cost_EPC_Labor_Plant'][0] + Econ_Parameters['Cost_EPC_Labor_Dist'][0]

    Cost_Dev = Econ_Parameters['Cost_Dev_land'][0] + Econ_Parameters['Cost_Dev_EIA'][0] + \
               Econ_Parameters['Cost_Dev_connection'][0] + Econ_Parameters['Cost_Dev_ICT'][0] + \
               Econ_Parameters['Cost_Dev_contingency'][0] + Econ_Parameters['Cost_Dev_overhead'][0] + \
               Econ_Parameters['Cost_taxes'][0]
    ...
    LoanPrincipal, year, Cost, Revenue, CashonHand, Balance, M, O, tariff = mcashflow(tariff_hillclimb_multiplier,
                                                                                      lifetime, f_pv, a_pv, f, a,
                                                                                      Batt_life_yrs, equity_debt_ratio,
                                                                                      term, loadkWh, interest_rate,
                                                                                      loanfactor, PVkW, BattKWh, LEC,
                                                                                      C1_pv, C1_LPG, Cost_bank,
                                                                                      Cost_Propane_yr)
...
if __name__ == "__main__":
    ...
    LoanPrincipal, year, Cost, Revenue, CashonHand, Balance, M, O, tariff, Batt_life_yrs = Econ_total(propane, PVkW,
                                                                                                      BattKWh,
                                                                                                      Batt_kWh_tot,
                                                                                                      peakload, loadkWh)
